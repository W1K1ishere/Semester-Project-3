    ...
    <div class="pl-20 flex flex-row justify-between">
        ...
        <div class="flex flex-col gap-5 pt-32">
            @guest
                ...
                <a href="/login" class="bg-black text-white px-4 py-2 w-24 text-center rounded-xl">Login</a>
            @endguest
            @auth
                <p class="text-5xl font-bold">Explore your Tabely account</p>
                <p class="opacity-50">Tabely is making your everyday life easier</p>
                <a href="/profile/{{ Auth()->user() }}" class="bg-black text-white px-4 py-2 w-24 text-center rounded-xl">Explore</a>
            @endauth
        </div>
        {{--right side image--}}
