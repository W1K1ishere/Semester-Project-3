...
class CheckTimeAndRunCommandEndBreak extends Command
{
    protected $signature = 'wifi2ble:checktimebreakend';
...
$departments = DB::table('departments')
    ->where('break_time_end', $now)
    ->get();

Log::info("Departments found:", $departments->toArray());

if ($departments->isEmpty()) {
    Log::info("No departments scheduled to end break at {$now}");
    return;
}
